...
#' txt_files <- list.files("RAW-DATA-8o-DISME",
#'                         recursive = TRUE, 
#'                         full.names = TRUE) %>%
#'    discard(., ~str_detect(.x, "\\.zip")) %>%
#'    keep(., ~str_detect(.x, "A838|A852"))
#'# missing columns problem example
#'  A838_problems <- import_txt_files_inmet(txt_files[1], 
#'                                          verbose = TRUE, 
#'                                          only.problems = TRUE)
#'  A838_data <- import_txt_files_inmet(txt_files[1], 
#'                                      verbose = TRUE, 
#'                                      only.problems = FALSE)
#'  View(slice(A838_data, A838_problems$row))
#'# too much columns problem example
#'# repeat for txt_files[2]
